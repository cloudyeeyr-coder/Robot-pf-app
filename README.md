...
### 🛡️ 랜딩페이지 (S.H.I.E.L.D. 고객 Hook 단계)
`/` 경로에 **6중 AI 감리 시스템(S.H.I.E.L.D.) 컨셉** 기반의 랜딩페이지가 배치되어 있습니다.  
*Solvency(재무감시) · Hardware(기술검증) · Investment(ROI분석) · Escrow(자금통제) · Local A/S(관제) · Dispute(분쟁예측)* — 6개 특화 AI 에이전트가 교차 검증하는 프레임워크로, 단순 기능 나열을 하나의 강력한 브랜드 아이덴티티로 통합했습니다.  
실제 인터뷰 인용(조상필/김도진) + 3 페르소나 맞춤 CTA + 실시간 대기 카운터 + 선착순 긴급성 장치 포함.
...
├── pages/
│   ├── Landing.tsx                # ★ 랜딩페이지 (/ 경로, 고객 Hook)
│   ├── Home.tsx                   # 서비스 홈 (/home 경로, CTA 진입점)
│   ├── admin/                     # 관리자 전용 페이지 (5개)
│   ├── manufacturer/              # 제조사 전용 페이지 (3개)
│   ├── partner/                   # SI 파트너 전용 페이지 (3개)
│   ├── as/                        # AS 티켓 페이지
│   └── *.tsx                      # 공통 페이지 (Search, Calculator 등)
...
| **랜딩페이지 체크리스트** | `docs/LANDING_PAGE_CHECKLIST.md` | 사람/AI | A유형(불안 해소형) 전략 기반 랜딩페이지 평가 (12/12 달성) |
